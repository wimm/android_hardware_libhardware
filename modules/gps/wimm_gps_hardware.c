...
//          nmea_reader_update_accuracy(r, tok_accuracy);
    ...
    if (!gps_state->first_fix &&
      ...
      //  ath_send_ni_notification(r);   //sam changed
 ...
 //   if ( isatty( state->fd ) ) {
  //      struct termios  ios;
   ...
   // }
	...
	state->thread = state->callbacks.create_thread_cb("wimm-gps-rfkill", gps_state_thread, state);
        if (!state->thread)
	{
...
static const void* athr_gps_get_extension(const char* name)
{
    D("Tori:athr_gps_get_extension : %s",name);
//Tori Add
...
//Tori add for rfkill
    ...
    int size;
    init_rfkill();
    LOGD("Tori debug: GPS power on!");
    if((fd = open(rfkill_state_path, O_WRONLY)) < 0)
    {
